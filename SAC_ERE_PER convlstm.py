...
class Actor(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed, hidden_size=256, init_w=3e-3, log_std_min=-20, log_std_max=2):
        ...
        self.convlstm_layers = ConvLSTM(input_dim=3,
                                    hidden_dim=[4, 4, 8],
                                    kernel_size=(3, 3),
                                    num_layers=3,
                                    batch_first=True,
        ...
        self.linear_layers2 = nn.Sequential(
            nn.Linear(in_features = 8*64*64, out_features = 128),
            nn.Tanh(),
            nn.Linear(128, 128),
            nn.Tanh(),
            nn.Linear(128, 2)
    ...
    def forward(self, state):
        _, x = self.convlstm_layers(state)
        x_flat = self.flatten(x[0][1])
        # x = self.cnn_layers(state)
        mu = self.linear_layers1(x_flat)
        log_std = self.linear_layers2(x_flat)
    ...
    def get_action(self, state):
        ...
        mu, log_std = self.forward(state.unsqueeze(0).unsqueeze(0))
        std = log_std.exp()
...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        next_action, log_pis_next = self.actor_local.evaluate(next_states.unsqueeze(1))
        ...
        # Compute alpha loss
        actions_pred, log_pis = self.actor_local.evaluate(states.unsqueeze(1))
