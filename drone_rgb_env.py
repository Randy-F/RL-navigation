...
class drone_env_heightcontrol(drone_env):
	def __init__(self,start = [0,0,-4],aim = [65, 0, -4],scaling_factor = 5,img_size = [64,64], max_eps_step = 40):
		drone_env.__init__(self,start,aim)
		self.scaling_factor = scaling_factor
		self.aim = np.array(aim)
		...
		self.csvUtil = CsvUtil(path = "path.csv")
	...
	def move(self, action):
		...
		x = pos.x_val + 4
		y = pos.y_val + 0
		z = pos.z_val - action[0] * self.scaling_factor
		
		self.client.moveByVelocityZAsync(2.5,0,z,5)
		
		self.csvUtil.write([pos.x_val,pos.y_val,pos.z_val])
	...
	def step(self,action): 
		...
		if info == "Collision":
			norm_state = np.zeros((3,64,64))
            # norm_state[0] = np.zeros((64,64,1))
		if IMAGE_VIEW:
			cv2.imshow("view",np.transpose(norm_state,(1,2,0)))
	...
	def getImg(self):
		try:
			...
			img1d = np.array(airsim.string_to_uint8_array(responses[0].image_data_uint8), dtype=np.float)
...
class drone_env_heightcontrol2d(drone_env_heightcontrol):
	def move(self, action):
		_state = self.client.simGetGroundTruthKinematics()
		pos = _state.position
		...
		self.client.moveByVelocityZAsync(x,0,z,5)
